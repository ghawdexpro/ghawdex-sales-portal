/**
 * Send 24h Follow-up via Zoho CRM
 *
 * Usage: node scripts/send-test-24h-zoho.mjs <zoho_lead_id>
 *
 * Example: node scripts/send-test-24h-zoho.mjs 1234567890123456789
 */
...
const ZOHO_LEAD_ID = process.argv[2];

if (!ZOHO_LEAD_ID) {
  console.error('Usage: node scripts/send-test-24h-zoho.mjs <zoho_lead_id>');
  console.error('\nTo find a lead ID, go to Zoho CRM -> Leads -> click a lead');
  console.error('The ID is in the URL: /crm/org123/tab/Leads/1234567890123456789');
  process.exit(1);
}
...
async function sendEmailViaZohoCRM(leadId, subject, html) {
  const token = await getAccessToken();

  console.log(`\nSending email to lead ${leadId}...`);
...
async function main() {
  ...
  console.log('SENDING 24H FOLLOW-UP VIA ZOHO CRM');
  console.log('='.repeat(60));
  console.log(`Lead ID: ${ZOHO_LEAD_ID}`);
  ...
  try {
    const success = await sendEmailViaZohoCRM(ZOHO_LEAD_ID, subject, html);

    if (success) {
      console.log('\n✅ Email sent! Check the lead timeline in Zoho CRM.');
    } else {
