...
export default function SocialLogin({ onLogin, onError }: SocialLoginProps) {
  ...
  const isLoggedIn = state.isSocialLogin && state.email;
  ...
  return (
    <div className="mb-6">
      ...
      {/* Highlighted box for quick sign-in - fixed height to prevent layout shift */}
      <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-4 mb-4 min-h-[140px]">
        {isLoggedIn ? (
          /* Success state - shown inside the same container */
          ...
          <>
            ...
            <div className="space-y-3">
              {/* Google Sign-In Button - fixed height to match Google's large button */}
              {hasGoogle && (
                <div className="flex justify-center h-[44px] items-center overflow-hidden">
                  {googleLoaded ? (
                    <div id="google-signin-btn" className="flex justify-center" />
                  ) : (
                    <button
                      disabled
                      className="w-full h-[44px] flex items-center justify-center gap-3 bg-white/10 border border-white/20 rounded-lg px-4 text-gray-400"
                    >
