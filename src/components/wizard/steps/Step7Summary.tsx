  ...
  useEffect(() => {
    ...
    if (state.totalPrice && (state.selectedSystem || isBatteryOnly)) {
      ...
      trackTelegramWizardStep(6, 'Summary', {
        ...
        householdSize: state.householdSize ?? undefined,
        monthlyBill: state.monthlyBill ?? undefined,
        consumptionKwh: state.consumptionKwh ?? undefined,
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {state.contractSigningUrl && (
        <div className="relative bg-gradient-to-r from-green-500/20 via-emerald-500/20 to-green-500/20 border-2 border-green-500/60 rounded-2xl p-8 mb-6 overflow-hidden">
          ...
          <a
            ...
            className="relative w-full flex items-center justify-center gap-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold py-5 px-8 rounded-xl hover:shadow-xl hover:shadow-green-500/30 hover:scale-[1.02] transition-all text-lg"
          >
      ...
      <div className={`bg-white/5 border border-white/10 rounded-2xl p-5 mb-6 ${state.contractSigningUrl ? '' : 'bg-gradient-to-r from-[#25D366]/20 to-[#128C7E]/20 border-[#25D366]/40 p-6'}`}>
        <div className="text-center mb-3">
          <h3 className={`font-bold mb-1 ${state.contractSigningUrl ? 'text-gray-300 text-base' : 'text-white text-xl'}`}>
            {state.contractSigningUrl ? 'Have Questions?' : 'Ready to Go Solar?'}
          </h3>
        ...
        <div className={`flex gap-3 ${state.contractSigningUrl ? '' : 'flex-col'}`}>
          ...
          >
            ...
            <span>{state.contractSigningUrl ? 'Call Us' : 'Or call +356 7905 5156'}</span>
          </button>
        </div>
      </div>
          ...
          >
            ...
            <div className="sticky bottom-0 bg-gray-50 border-t border-gray-200 p-4">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                ...
                <button
                  onClick={() => {
                    const docType = modalOpen === 'proposal' ? 'Contract Proposal' : 'Technical Specs';
                    const message = `Hi! Check out my ${docType} from GhawdeX Solar:\n\n` +
                      `💰 Total Investment: ${formatCurrency(state.totalPrice || 0)}\n` +
                      `${state.selectedSystem ? `☀️ System: ${state.selectedSystem.name} (${state.selectedSystem.systemSizeKw} kWp)` : ''}\n` +
                      `${state.withBattery && battery ? `🔋 Battery: ${battery.capacityKwh} kWh` : ''}\n` +
                      `📍 Location: ${state.address}\n\n` +
                      `View full details: https://get.ghawdex.pro\n\n` +
                      `Quote Ref: ${quoteRef}`;
