 ...
 * Enforces 50% deposit OR €799 minimum, whichever is higher, but never exceeds total price
 * Used for all purchases: PV systems, batteries, bundles
 *
 * @param totalPrice Total price after grant deduction
 * @returns Deposit amount (50% or €799, whichever is higher, capped at total price)
 *
 * @example
 * calculateDeposit(1000) // Returns 799 (50% = €500, minimum is €799)
 * calculateDeposit(3000) // Returns 1500 (50% = €1500 > €799)
...
export function calculateDeposit(totalPrice: number): number {
  ...
  const fiftyPercent = totalPrice * 0.50;
  const desiredDeposit = Math.max(fiftyPercent, 799);
